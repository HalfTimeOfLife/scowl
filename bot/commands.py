"""
bot/commands.py — Discord event listeners and slash commands.

Covers:
- Welcome message on guild join
- Passive attachment listener: download, hash, analyze and report
...
from analysis.utils import compute_hashes, format_size, safe_filename
from config import (
    MAX_FILE_SIZE_BYTES,
    TEMP_DOWNLOAD_DIR,
...
class ScowlEvents(commands.Cog):
    ...
    async def on_guild_join(self, guild):
        ...
        if target_channel is not None:
            ...
            embed.add_field(name="Version", value="v0.2", inline=True)
    ...
    async def on_message(self, message):
        """Analyze any file attached to a message in a watched channel."""
        ...
        for attachment in message.attachments:
            if attachment.size > MAX_FILE_SIZE_BYTES:
                logger.warning(
                    f"file too large, skipped | id={attachment.id} "
                    f"filename={attachment.filename} size={attachment.size} "
            ...
            logger.info(
                f"file received | id={attachment.id} filename={attachment.filename} "
                f"size={attachment.size} author={message.author} channel={message.channel.name}"
            )
            file_info.path = dest
            ...
            logger.debug(
                f"hashes computed | id={attachment.id} sha256={hashes['sha256']} "
                f"sha1={hashes['sha1']} md5={hashes['md5']}"
            )
            ...
            # ---- Received embed ----
            received_embed = Embed(title="📥 File received", color=0x5865F2)
    ...
    async def help(self, interaction: Interaction):
        """Send a help embed listing scOWL's capabilities and commands."""
        embed = Embed(
        ...
        embed.add_field(
            ...
            value="PE · ELF · PDF · Office · PS1 / BAT / CMD / VBS / JS / SH",
            inline=False,
        )
        embed.add_field(name="Watching", value=self.watched, inline=False)
    ...
    async def status(self, interaction: Interaction):
        """Send a status embed with uptime, latency and server count."""
